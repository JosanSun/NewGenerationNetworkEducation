...
//根据数据库中节点信息生成节点
void knowledge::generateNodes(vector<QString> attributes, vector<vector<int>> successors, vector<vector<int>> predecessors, int r)
{
...
void knowledge::trackLearning()
{
	...
	//初始化
	for (int i = 0; i < nodesInPic.size(); i++)
	{
		nodesInPic[i].setIsLearned(false);
	}
	for (int i = 0; i < learningTrack.size(); i++)
	{
