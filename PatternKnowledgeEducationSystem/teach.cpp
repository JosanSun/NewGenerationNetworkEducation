	...
	: QWidget(parent)
{
	...
	init();
	QObject::connect(timer, SIGNAL(timeout()), this, SLOT(timeUpdateSlot()));//更新系统时间
	QObject::connect(ui.playAgainButton, SIGNAL(clicked()), this, SLOT(playAgainSlot()));//重新播放案例
	QObject::connect(ui.changeCaseButton, SIGNAL(clicked()), this, SLOT(changeCaseSlot()));//更换案例
	QObject::connect(ui.discussionButton, SIGNAL(clicked()), this, SLOT(goToDiscussionSlot()));//进入讨论区
	QObject::connect(ui.beginTestButton, SIGNAL(clicked()), this, SLOT(goToTestSlot()));//进入测试模块
	QObject::connect(ui.quitButton, SIGNAL(clicked()), this, SLOT(close()));//关闭系统
	timer->start(1000);
}
...
void teach::init(){
	...
	if (_first == "B"){//当前知识点是基本知识节点
		...
		while (query.next()){
			...
			else if (query.value(1).toInt() == 1){//后继
				QString _about = query.value(2).toString();
				QString _sep = ",";
				int inx = 1;
				int pos = _about.indexOf(_sep);				
