	...
	: QWidget(parent)
{
	...
	connect(knowledgeClickLabel, SIGNAL(clicked()), this, SLOT(goToKnowledgeWindowSlot()));//进入知识库查看模块
	connect(teachClickLabel, SIGNAL(clicked()), this, SLOT(goToTeachWindowSlot()));//进入教学模块
	connect(testClickLabel, SIGNAL(clicked()), this, SLOT(goToTestWindowSlot()));//进入测试模块
	connect(userClickLabel, SIGNAL(clicked()), this, SLOT(goToUserWindowSlot()));//进入用户管理模块
	connect(ui.quitButton, SIGNAL(clicked()), this, SLOT(close()));//关闭系统
	timer->start(1000);
}
...
void initial::goToTeachWindowSlot(){
	...
	if (_lastPoint == QStringLiteral("无")){//用户无历史学习数据
		QMessageBox msgBox;
	...
	connect(teachWindow, SIGNAL(destroyed()), this, SLOT(updateCurrentKidSlot()));
}
...
void initial::goToTestWindowSlot(){
	...
	connect(testWindow, SIGNAL(destroyed()), this, SLOT(updateCurrentKidSlot()));
}
